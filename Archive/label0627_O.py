...
OUTPUT_DIR = 'Archive/Archive/Final_Cleaned_Dataset_1606_Output_0627/'
...
# [NEW] Configuration for Redundancy Mismatch Crops
REDUNDANCY_CHECK_BASE = '/home/wanfangyuan/Desktop/Wan_Fangyuan/Sanwa/Sanwa Data2/ASTAR/Sanwadata/Cleaned_Results_Output12_16/Redundancy_Mismatch_Recheck/'
...
def copy_abnormal_crop(filename_png, roi_id, specific_dest_folder):
    """
    Copies the crop image to the specific destination folder.
    Returns True if successful, False otherwise.
    """
    try:
        # Determine folder name from filename (removes extension)
        folder_name_no_ext = os.path.splitext(filename_png)[0]
        
        # Source paths (try both jpg and png)
        ...
        # Target path: specific_dest_folder / FolderName / ROI.jpg
...
def process_single_file(input_csv_path, output_folder):
    ...
    abnormal_records = []
    redundancy_mismatch_records = [] # [NEW] List for mismatches in redundant rows
    ...
    # --- PHASE 3: TIME-STATE ANALYSIS & REDUNDANCY CHECK ---
    ...
    for i in range(len(rows_list)):
        ...
        if i > 0 and curr_content == prev_content:
            ...
            # [NEW] Deep Compare: If Fingerprint matches, check ALL ROI columns for exact match
            # This catches cases where the 'Main Data' is redundant, but a specific ROI (e.g. Time or Status) differs
            prev_row = rows_list[i-1]
            for roi_col in roi_map.keys():
                val_curr = str(curr_row.get(roi_col, '')).strip()
                val_prev = str(prev_row.get(roi_col, '')).strip()
    ...
    # 1. Hanged Log (Time Static + Unique Data)
    ...
    # 2. Abnormal Values Log (Format/Stat issues)
    if abnormal_records:
    ...
    # 3. [NEW] Redundancy Mismatch Log
    if redundancy_mismatch_records:
        df_mismatch = pd.DataFrame(redundancy_mismatch_records).drop_duplicates()
        log_path_mismatch = os.path.join(output_folder, f"{base_name}_Redundancy_Mismatch_Log.csv")
        df_mismatch.to_csv(log_path_mismatch, index=False)
        
        # Create folder for Redundancy Mismatches
        mismatch_crop_folder = os.path.join(REDUNDANCY_CHECK_BASE, base_name)
        if not os.path.exists(mismatch_crop_folder): os.makedirs(mismatch_crop_folder)
        
        count_mismatch = sum(1 for _, rec in df_mismatch.iterrows() if copy_abnormal_crop(rec['Filename_Current'], rec['ROI_ID'], mismatch_crop_folder))
        print(f"  -> [Attention] Redundancy Mismatches found: {len(df_mismatch)}. Images copied to new folder: {count_mismatch}")
