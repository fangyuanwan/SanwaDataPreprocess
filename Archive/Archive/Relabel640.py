...
# 1. INPUT: The folder containing your "Final_7B_Corrected" files
INPUT_FOLDER = 'Archive/Archive/Final_Cleaned_Dataset_1606_Output_0640/Final_7B_Corrected/'

# 2. OUTPUT: Where the re-labeled files will go
OUTPUT_DIR = 'Archive/Archive/Final_Cleaned_Dataset_1606_Output_0640/Final_7B_Relabeled_Verified/'

# 3. Crop locations (Pointed to new folders to separate from previous steps)
MANUAL_CHECK_BASE = '/home/wanfangyuan/Desktop/Wan_Fangyuan/Sanwa/Sanwa Data2/ASTAR/Sanwadata/Cleaned_Results_Output12_16/Abnormal_Crops_Recheck_Final/'
...
REDUNDANCY_CHECK_BASE = '/home/wanfangyuan/Desktop/Wan_Fangyuan/Sanwa/Sanwa Data2/ASTAR/Sanwadata/Cleaned_Results_Output12_16/Redundancy_Mismatch_Recheck_Final/'
...
def process_single_file(input_csv_path, output_folder):
    ...
    # Handle filename adjustment if input was already "_7B_Corrected" or similar
    # We want a clean base name
    clean_base = base_name.replace("_7B_Corrected", "").replace("_Cleaned", "")
    ...
    # --- [NEW STEP] RENAME OLD LABELS ---
    # We rename the existing status columns to "_Legacy" to preserve them for benchmarking
    rename_map = {
        'Time_Status': 'Time_Status_Legacy',
        'Data_Redundancy': 'Data_Redundancy_Legacy',
        'Matched_File': 'Matched_File_Legacy',
        'Duration_Since_Change': 'Duration_Legacy'
    }
    df_clean.rename(columns=rename_map, inplace=True)
    print("  -> Renamed old status columns to '_Legacy'")

    # Init new columns (These will be recalculated based on 7B data)
    ...
    # --- PHASE 1: BASIC VALIDATION (Re-check validity after AI fix) ---
    ...
    # --- PHASE 3: RE-RUN TIME & REDUNDANCY ANALYSIS ---
    print(f"  Re-Analyzing Similarity on Corrected Data...")
    ...
    for i in range(len(rows_list)):
        ...
        if i > 0 and similarity_prev >= SIMILARITY_THRESHOLD:
            ...
            # Check for Mismatches (Should be fewer now that AI fixed them!)
            for k in range(len(curr_vals)):
                if curr_vals[k] != prev_vals[k]:
                    redundancy_mismatch_records.append({
                        ...
                        'Reason': 'Redundant Row Value Mismatch (Post-Fix)'
                    })
    ...
    # Save Main Relabeled CSV
    out_name = f"{clean_base}_Relabeled_Verified.csv"
    df_clean.to_csv(os.path.join(output_folder, out_name), index=False)
    ...
    if not hanged.empty:
        hanged[['Filename', 'ROI_52', 'Time_Status', 'Data_Redundancy']].to_csv(
            os.path.join(output_folder, f"{clean_base}_Hanged_Logic_Log_New.csv"), index=False)

    # 2. Abnormal Values Log (Should be empty or very small if 7B worked well)
    if abnormal_records:
        df_abn = pd.DataFrame(abnormal_records).drop_duplicates()
        df_abn.to_csv(os.path.join(output_folder, f"{clean_base}_Abnormal_Log_New.csv"), index=False)
        if not os.path.exists(MANUAL_CHECK_BASE): os.makedirs(MANUAL_CHECK_BASE)
        cnt = sum(1 for _, r in df_abn.iterrows() if copy_abnormal_crop(clean_base, r['Filename'], r['ROI_ID'], MANUAL_CHECK_BASE))
        print(f"  -> Remaining Value Issues: {len(df_abn)}. Images copied: {cnt}")

    # 3. Redundancy Mismatch Log (Should be empty if all conflicts resolved)
    if redundancy_mismatch_records:
        df_mis = pd.DataFrame(redundancy_mismatch_records).drop_duplicates()
        df_mis.to_csv(os.path.join(output_folder, f"{clean_base}_Redundancy_Mismatch_Log_New.csv"), index=False)
        if not os.path.exists(REDUNDANCY_CHECK_BASE): os.makedirs(REDUNDANCY_CHECK_BASE)
        cnt_mis = sum(1 for _, r in df_mis.iterrows() if copy_abnormal_crop(clean_base, r['Filename_Current'], r['ROI_ID'], REDUNDANCY_CHECK_BASE))
        print(f"  -> Remaining Mismatches: {len(df_mis)}. Images copied: {cnt_mis}")

    print(f"  -> Processed {len(df)} rows. Saved to {out_name}")
