...
# If you are using a custom port (e.g. 11436), uncomment the next line:
...
# Python ranges are exclusive at the end (e.g., range(0, 8) gives 0,1,2,3,4,5,6,7)
CSV_GROUPS = {
...
class GPUHandler(FileSystemEventHandler):
    ...
    def clean_machine_time(self, t1, t2):
        ...
        raw = f"{t1} {t2}".strip()
        if not t1 or t1 == "NA": return raw
        
        # Attempt to clean common OCR issues in dates
    ...
    def process_single_roi(self, args):
        ...
        with print_lock:
            # Print real value for Timestamps (117, 118), dot for others
    ...
    def run_parallel_pipeline(self, img_path, save_dir, relative_parent):
        ...
        # 2. Machine Timestamp (Join 117 + 118)
    ...
    def ask_ollama_single(self, image_path):
        ...
        try:
            # If using custom port, use: client = ollama.Client(host=...)
    ...
    def append_to_summary_csv(self, csv_name, id_list, results_dict, filename, machine_ts, file_ts, relative_parent):
        ...
        # --- HEADER CONSTRUCTION ---
        # 1. Machine Timestamp (117+118)
        # 2. File Timestamp (Filename)
        # 3. Original Filename
        # 4. Data Columns...
        ...
        # Ensure 117/118 are logged in the raw data columns if they aren't part of the main group?
        # Based on your request, they are the first column. 
        # I will NOT add them as extra columns at the end unless specifically requested in the group range.
