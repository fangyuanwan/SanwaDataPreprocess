...
# 🔴 RESUME CONFIGURATION (Change this filename to restart from a different point)
START_FROM_FILE = "2025-12-16 17.48.47.png"
...
def main():
    ...
    print(f"   Resume Target: {START_FROM_FILE}")
    ...
    # Sort ensures we find the file in the correct order
    image_files.sort()
    ...
    # If START_FROM_FILE is empty, start immediately (legacy behavior)
    ...
    for i, img_path in enumerate(image_files):
        # 1. Check if we have hit the start file yet
        if not start_processing:
            if img_path.name == START_FROM_FILE:
                start_processing = True
                found_start_file = True
                print(f"\n✅ FOUND RESUME POINT: {img_path.name}")
                print(f"   (Skipped {i} files prior to this)")
            else:
                # OPTIONAL: Print a dot every 100 skipped files just to show life
                if i % 100 == 0:
                    print(f"\rSkipping old files... [{i}/{total}]", end="", flush=True)
    ...
    if not found_start_file and START_FROM_FILE:
        print(f"\n⚠️ WARNING: Could not find the resume file: {START_FROM_FILE}")
        print("   (Check the filename spelling. No files were processed in batch mode.)")
