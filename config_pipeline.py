...
MISMATCH_PROMPTS = {
    'STATUS': (
        ...
        "  - GREEN text or starts with O → Output: OK\n"
        "  - RED text or starts with N → Output: NG\n"
        "  - Blank/Unreadable → Output: NG (fail-safe)\n"
        "\n"
        "Critical Rules:\n"
        "1. Output ONLY 'OK' or 'NG' (no NA allowed)\n"
        "2. GREEN text = OK, RED text = NG\n"
        "3. Starts with O (O, OH, OK) → OK\n"
        "4. Starts with N (N, NG, NH, NO, NaN) → NG\n"
        "5. If display is blank/dark → Output: NG (fail-safe)\n"
        "\n"
        "Think: What does it start with? O=OK, N=NG"
    ),
    'INTEGER': (
